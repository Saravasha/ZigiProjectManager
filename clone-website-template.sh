...
git_init_and_push() {
    ...
    git add .
    git commit -m "Initial commit for $repo_name [skip ci]"
    gh repo create "$REPO_OWNER/$repo_name" --private --source="$dir" --push
...
create_new_profile() {
    ...
    [[ "$USE_ORG" =~ ^[Yy]$ ]] && read -rp "Org name: " GITHUB_ORG || GITHUB_ORG=""
...
remove_profile() {
    ...
    select profile_file in "${profiles[@]}"; do
        ...
        [[ "$confirm" =~ ^[Yy]$ ]] && rm -f "$profile_file" && success "Removed $profile_file" || info "Cancelled"
        break
    done
}
...
use_profile() {
    ...
    REPO_OWNER=${GITHUB_ORG:-Saravasha}
    EMAIL="siavash.gosheh@gmail.com"

    # Handle old profiles that lack project_dir
    PROJECT_DIR=$(jq -r '.project_dir // empty' "$PROFILE_JSON")
    if [[ -z "$PROJECT_DIR" ]]; then
        ...
        # Persist the absolute path back to the JSON
        jq --arg project_dir "$PROJECT_DIR" '. + {project_dir: $project_dir}' \
           "$PROFILE_JSON" > "$PROFILE_JSON.tmp" && mv "$PROFILE_JSON.tmp" "$PROFILE_JSON"
        info "Updated profile with absolute project path: $PROJECT_DIR"
    fi
...
init_frontend_repo() {
    ...
    find . \( -name "*.jsx" -o -name "*.html" \) -type f -exec sed -i "s/__PROJECT_NAME__/${PROJECT_NAME}/g" {} +

    for file in .env.staging .env.production; do
        [[ -f "$file" ]] && sed -i "s/__DOMAIN__/${sed_safe_domain}/g" "$file"
    done
    ...
    gh repo create "$REPO_OWNER/$FRONTEND_NAME" --private --source="$frontend_path" --push
...
init_backend_repo() {
    ...
    # Replace tokens in workflow files
    for file in .github/workflows/*.yml; do
        [[ -f "$file" ]] && sed -i "s/__BACKEND_NAME__/${BACKEND_NAME}/g" "$file"
    done

    # Other token replacements in code/config
    declare -A TOKEN_REPLACEMENTS=( ["__PROJECT_NAME__"]="$PROJECT_NAME" ["__DOMAIN_NAME__"]="$DOMAIN_NAME" )
    local FILES_TO_REPLACE=("ApplicationDbContext.cs" "_Layout.cshtml" "Login.cshtml")
    for filename in "${FILES_TO_REPLACE[@]}"; do
        local file
        file=$(find . -name "$filename" | head -n 1)
        [[ -n "$file" ]] || continue
    ...
    for file in appsettings.Staging.json appsettings.Production.json; do
        [[ -f "$file" ]] && sed -i "s/DOMAIN_NAME/${DOMAIN_NAME}/g" "$file"
    done
    ...
    if [[ -d "$migrations_dir" ]] && find "$migrations_dir" -type f -iname "*init*.cs" | grep -q .; then
        ...
        info "⚡ No init migration found. Creating one..."
        pushd "$backend_path" >/dev/null || exit 1
        dotnet ef migrations add init --context ApplicationDbContext --output-dir Migrations
        success "Init migration created."
        popd >/dev/null || exit 1
    fi

    # Update DB unless on WSL
    if grep -qi microsoft /proc/version &>/dev/null; then
        info "⚡ Skipping database update on WSL (LocalDB not supported)"
    else
    ...
    # GitHub repo creation & push
    gh repo create "$REPO_OWNER/$BACKEND_NAME" --private --source="$backend_path" --push
    for branch in dev stage; do
        git checkout -b "$branch"
        git push -u origin "$branch"
    done
...
detect_runtimes() {
    ...
    # Optional debug
    info "[DEBUG]: Node version detected: '$node_version'"
...
setup_project() {
    ...
    REPO_OWNER=${GITHUB_ORG:-Saravasha}  # static default
    EMAIL="siavash.gosheh@gmail.com"

    setup_project_structure
    init_frontend_repo
    init_backend_repo
    success "🎉 Project [$PROJECT_NAME] setup complete under owner [$REPO_OWNER]."
}
...
# === Menu Items ===
options=("Create new profile" "Remove a profile" "Use existing profile" "Detect Runtimes" "Setup Project from Profile" "Exit")
